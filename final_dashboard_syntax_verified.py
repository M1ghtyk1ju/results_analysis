...
df['Weak Subjects'] = df.apply(
    lambda row: sum(
        1 for s in subject_columns
        if pd.notna(row.get(f"{s}_AL")) and
...
# Compute Weak Subjects count safely and correctly
df['Weak Subjects'] = df.apply(
    lambda row: sum(
        1 for s in subject_columns
        if pd.notna(row.get(f"{s}_AL")) and
...
def highlight_weak_subjects(row):
    al_numeric = [al_to_numeric(row[f"{s}_AL"]) for s in subject_columns if f"{s}_AL" in row and pd.notna(row.get(f"{s}_AL"))]
    if not al_numeric:
...
st.dataframe(
    sorted_df.style.apply(
        highlight_weak_subjects, axis=1, subset=[f"{s}_AL" for s in subject_columns if f"{s}_AL" in sorted_df.columns]
    ),
